-- Create MasterDataItem table for ordering sizes, categories, departments (if not exists)
...
CREATE INDEX IF NOT EXISTS "master_data_items_type_order_idx" ON "master_data_items"("type", "order");

-- Insert predefined sizes (only if not already present)
INSERT OR IGNORE INTO "master_data_items" ("id", "type", "value", "order", "createdAt", "updatedAt") VALUES
...
-- Insert predefined categories (only if not already present)
INSERT OR IGNORE INTO "master_data_items" ("id", "type", "value", "order", "createdAt", "updatedAt") VALUES
