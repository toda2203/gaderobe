...
/**
 * GET /api/backup-config/available-backups
 * List all available automatic backups
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const files = fs.readdirSync(backupDir).filter((file: string) => file.endsWith('.zip'));
...
/**
 * POST /api/backup-config/restore-backup/:filename
