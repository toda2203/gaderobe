...
/**
 * GET /api/system/company-logo
 * Get current company logo (global)
 */
router.get(
  '/company-logo',
  asyncHandler(async (req: AuthenticatedRequest, res) => {
    const uploadsDir = path.join(config.uploadDir, 'clothing-images');
    ...
    const logoUrl = logoFile ? `/uploads/clothing-images/${logoFile}` : null;
  ...
  asyncHandler(async (req: AuthenticatedRequest, res) => {
    ...
    const [
      ...
      prisma.employee.findMany({
        where: {
          ...
          currentClothing: {
            some: {
              status: { in: ['ISSUED', 'IN_USE', 'PENDING'] },
            },
          },
        },
        select: {
      ...
      prisma.employee.findMany({
        where: {
          status: 'ACTIVE',
          currentClothing: { none: {} },
          personalizedClothing: { none: {} },
        },
        select: {
          id: true,
          firstName: true,
    ...
    res.json({
      ...
      data: {
        ...
        inactiveEmployeesWithItems: {
          total: inactiveEmployeesWithItems.length,
          employees: inactiveEmployeesWithItems.map((employee) => ({
            id: employee.id,
            name: `${employee.firstName} ${employee.lastName}`,
            email: employee.email,
            itemCount: employee.currentClothing.length,
            items: employee.currentClothing.map((item) => ({
              id: item.id,
        ...
        employeesWithoutClothing: {
          total: employeesWithoutClothing.length,
          employees: employeesWithoutClothing.map((employee) => ({
            id: employee.id,
            name: `${employee.firstName} ${employee.lastName}`,
            email: employee.email,
            department: employee.department || 'Nicht zugewiesen',
          })),
        },
        pendingConfirmations: {
          total: pendingConfirmationsCount,
          confirmations: pendingConfirmations.map((conf) => ({
            id: conf.id,
