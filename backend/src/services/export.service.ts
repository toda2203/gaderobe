...
export class ExportService {
  ...
  async importFromCSVZip(buffer: Buffer): Promise<{
    ...
    try {
      ...
      // Note: The ZIP contains 'uploads/...' structure, so we extract directly into uploadsDir
      // to avoid creating /app/uploads/uploads/
      if (zip.folder('uploads')) {
        const uploadsFolder = zip.folder('uploads')!;
        for (const [filename, file] of Object.entries(uploadsFolder.files)) {
          if (!file.dir && filename.startsWith('uploads/')) {
            // Remove 'uploads/' prefix to get relative path
            const relativePath = filename.substring('uploads/'.length);
