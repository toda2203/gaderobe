...
export class TransactionService {
  ...
  async issueBulkClothing(input: BulkIssueClothingInput) {
    try {
      ...
      const retiredItems = clothingItems.filter((item) => item.status === 'RETIRED' || item.condition === 'RETIRED');
      if (retiredItems.length > 0) {
        throw new Error(
          `Some items are retired and cannot be issued: ${retiredItems.map((i) => i.internalId).join(', ')}`
        );
      }
  ...
  async issueClothing(input: IssueClothingInput) {
    try {
      ...
      if (clothingItem.status === 'RETIRED' || clothingItem.condition === 'RETIRED') {
        throw new Error('Clothing item is retired and cannot be issued');
      }
      ...
      const updatedTransactions = await prisma.$transaction(async (tx) => {
        ...
        for (const transaction of transactions) {
          ...
          await tx.clothingItem.update({
            ...
            data: {
              status: input.conditionOnReturn === 'RETIRED' ? 'RETIRED' : 'AVAILABLE',
      ...
      const updatedTransactions = await prisma.$transaction(async (tx) => {
        ...
        for (const transaction of transactions) {
          ...
          await tx.clothingItem.update({
            where: { id: transaction.clothingItemId },
            data: {
              status: itemData.conditionOnReturn === 'RETIRED' ? 'RETIRED' : 'AVAILABLE',
