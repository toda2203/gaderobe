...
const ClothingItemsPage: React.FC = () => {
  ...
  const [hideRetired, setHideRetired] = useState<boolean>(() => {
  ...
  // Save hideRetired preference to localStorage
  useEffect(() => {
    localStorage.setItem('clothingItems_hideRetired', JSON.stringify(hideRetired));
  }, [hideRetired]);
  ...
  useEffect(() => {
    ...
    // Hide retired items filter
    if (hideRetired) {
      filtered = filtered.filter((item) => item.status !== 'RETIRED');
    }

    setFilteredItems(filtered);
  }, [clothingItems, searchText, filterStatus, filterCondition, filterCategory, filterType, hideRetired]);
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={6}>
            <Space>
              <Switch
                checked={hideRetired}
                onChange={(checked) => setHideRetired(checked)}
              />
              <span>Ausgemusterte ausblenden</span>
            </Space>
          </Col>
          <Col xs={24} sm={12} md={6}>
            <Button
              onClick={() => {
                ...
                setFilterCategory('');
                setFilterType('');
                setHideRetired(true); // Reset to default
              }}
            >
              Filter zurücksetzen
            </Button>
